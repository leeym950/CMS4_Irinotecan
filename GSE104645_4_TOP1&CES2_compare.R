...
subset <- cbind(t(exp.data)[ ,c("TOP1", "CES2")], ntp.result)
...
p <- ggboxplot(data=subset, "CMS4", "CES2", add="jitter")
...
p <- ggboxplot(data=FDR.filtered.subset, "CMS4.filtered", "TOP1", add="jitter")
p+ stat_compare_means()

p <- ggboxplot(data=FDR.filtered.subset, "CMS4.filtered", "CES2", add="jitter")
p+ stat_compare_means()
